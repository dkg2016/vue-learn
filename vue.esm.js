...
// 进行一次完全 normalization，以满足所有可能类型的子值。
function normalizeChildren (children) {
  return isPrimitive(children)
    ? [createTextVNode(children)] // 返回文本节点
    : Array.isArray(children)     // 是否数组
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    try {
      // 调用 vm 上的 render 函数，并把vm作为this，vm.$createElement 作为参数传入
...
// patch 是一个函数
// patch 函数是执行 createPatchFunction 的返回结果
var patch = createPatchFunction({ nodeOps: nodeOps, modules: modules });
